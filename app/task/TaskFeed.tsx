...
//   // Inside your TaskFeed Page Component
//   const [selectedTask, setSelectedTask] = useState<Task | null>(null);

//   // Handler when "View Gig" is clicked on a Card
//   const handleViewGig = (task: Task) => {
//     setSelectedTask(task);
//     setIsPopupOpen(true);
//   };

//   // Handler when task is successfully accepted
//   const handleTaskAccepted = () => {
//     // Re-fetch your tasks here to remove the accepted one from the feed
//     fetchTasks();
//   };
...
export default function TaskFeed() {
  ...
  // Popup States
  const [isCreateOpen, setIsCreateOpen] = useState(false); // For Creating Tasks
  const [isViewOpen, setIsViewOpen] = useState(false);     // For Viewing Tasks
  const [selectedTask, setSelectedTask] = useState<Task | null>(null);
  ...
  const fetchTasks = async (query = "") => {
    ...
    try {
      ...
      const res = await fetch(url, {
         headers: {
             'Authorization': `Bearer ${localStorage.getItem('accessToken')}` // Ensure auth if needed
         }
      });
  ...
  // 2. Open "View Gig" Popup
  const handleViewGig = (task: Task) => {
    setSelectedTask(task);
    setIsViewOpen(true);
  };

  // 3. Refresh after Accepting
  const handleTaskAccepted = () => {
    fetchTasks(searchQuery); // Reload list to remove the accepted task
  };
      ...
      <div className="container mx-auto px-4 md:px-8 py-8">
        ...
        <div className="legacy-reveal grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 p-5 pb-24 border-2 border-black border-dashed min-h-[400px] bg-gray-50/50">
          {loading ? (
            ...
            // ✅ Render Task Cards with Click Handler
            tasks.map((task) => (
              <TaskCard 
                 key={task.id} 
                 task={task} 
                 onViewClick={handleViewGig} // <--- Pass the handler here
              />
            ))
            ...
            <div className="col-span-full flex flex-col items-center justify-center font-mono text-xl text-gray-400 h-64">
              ...
              <button 
                ...
                className="text-sm mt-2 underline hover:text-black uppercase"
              >
                Be the first to post
              </button>
            </div>
