...
import { Search, Shield, Clock, Plus, Globe, Briefcase, User, Phone, CheckCircle, X, QrCode } from 'lucide-react';
...
export default function NeoHelperDashboard() {
    ...
    const handleCompleteClick = (gigId: number) => {
                            ...
                            <div className="grid gap-4">
                                {acceptedGigs.map((task) => (
                                    <GigCard
                                        ...
                                        onComplete={() => handleCompleteClick(task.id)} // Pass trigger function
                                    />
                                ))}
                            </div>
                        )}
...
// ✅ NEW MODAL COMPONENT FOR PAYMENT QR
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white border-[3px] border-black p-8 max-w-sm w-full shadow-[16px_16px_0px_0px_rgba(255,255,255,0.2)] relative">
                ...
                <div className="text-center">
                    ...
                    <h2 className="text-2xl font-black uppercase mb-2">Payment Required</h2>
                    <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-6">
                        Gig ID: #{gigId || '000'} // Status: Pending
                    </p>

                    <div className="border-[3px] border-black p-4 bg-gray-50 mb-6 flex justify-center">
                        {/* Placeholder QR Code - Replace with real logic if needed */}
                        <img 
                            src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=PaymentForGig_${gigId}`} 
                            alt="Payment QR" 
                            className="w-40 h-40 mix-blend-multiply"
                        />
                    </div>

                    <p className="text-[10px] font-bold uppercase mb-6 text-center leading-relaxed">
                        Scan this QR code to complete payment transaction.<br/>
                        Funds will be transferred to your wallet instantly.
                    </p>

                    <button 
                        onClick={onClose}
                        className="w-full bg-black text-white py-3 font-black uppercase text-sm hover:bg-green-600 transition-colors"
                    >
                        Done
                    </button>
                </div>
...
interface GigCardProps {
    ...
    isMyTask: boolean; 
    onComplete?: () => void; // New Prop for triggering completion
}

function GigCard({ status, title, location, count, assigneeName, assigneePhone, isMyTask, onComplete }: GigCardProps) {
    return (
        <div className="border-[3px] border-black p-6 bg-white transition-all cursor-pointer group shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-none translate-x-[-2px] translate-y-[-2px] hover:translate-x-0 hover:translate-y-0 relative overflow-hidden">
            ...
            {/* --- 👇 NEW: COMPLETED BUTTON (Only for accepted gigs) 👇 --- */}
            ...
            {/* --- OWNER VIEW: Shows assignee info --- */}
